#!/usr/bin/env bash
# Pre-uninstall script for Librarian Subagent Plugin
...
echo "============================================"
echo "Uninstalling Librarian Subagent Plugin..."
echo "============================================"
echo ""
...
echo "  - Knowledge entries: ${CONFIG_DIR}/knowledge/"
echo "  - Search cache: ${CACHE_DIR}/"
echo ""
echo "  Choose an option:"
echo "  1. Keep all user data (recommended)"
echo "  2. Keep configuration and knowledge, remove cache"
...
case "$UNINSTALL_OPTION" in
    1)
        ...
        echo "  ✓ Knowledge entries preserved: ${CONFIG_DIR}/knowledge/"
        ;;
    2)
        echo ""
        echo "Removing cache, keeping configuration and knowledge..."
        if [[ -d "$CACHE_DIR" ]]; then
            rm -rf "$CACHE_DIR"
            echo "  ✓ Cache removed"
        fi
...
for prefix in "/usr/local/bin" "/data/data/com.termux/files/usr/bin" "$HOME/.local/bin"; do
    if [[ -L "${prefix}/oml-librarian" ]]; then
        rm -f "${prefix}/oml-librarian"
        echo "  ✓ Removed symlink: ${prefix}/oml-librarian"
    fi
done
...
echo ""
echo "============================================"
echo "Librarian Subagent Plugin uninstalled"
echo "============================================"
echo ""
